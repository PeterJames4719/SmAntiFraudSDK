 ...
 * 成功回调函数
 */
@required - (void)onSuccess:(NSString*) serverId;

/**
 * 异常回调函数
 */
@required - (void)onError:(NSInteger) errorCode;
@end
...
//    OnServerIdReceive _callback;
    BOOL _first;
    NSString *_appId;
    id<ServerSmidProtocol> _delegate;
    BOOL _encrypt;
}
...
@property(readwrite) BOOL first;
@property(readwrite) NSString *appId;
@property(readwrite) id<ServerSmidProtocol> delegate;
...
@interface SmAntiFraud : NSObject {
    id<ServerSmidProtocol> _idDelegate;
}
@property id<ServerSmidProtocol> idDelegate;
